...
((global) => {
   class ColorTransform {
      ...
      hexNormalize(str) {
         const regex = /^#([a-f\d]{3}|[a-f\d]{4}|[a-f\d]{6}|[a-f\d]{8})$/i;
      ...
      if (require.main === module) {
         ...
         if (args.includes('-v')) {
            ...
            for (let arg of args) {
               ...
               if (match) argMap[match[1]] = argMap[match[1]] = match[2];
            }
            if (argMap.c) {
               let color = argMap.c;
               let t = C.test(color);
               let hex = C.toHex(color);
               let rgb = C.toRgb(color);
               let hsl = C.toHsl(color);
               let ANSIFG = C.toANSIFG(color);
               let ANSIBG = C.toANSIBG(color);
               let preview = C.toANSIFG(color) + '██' + '\x1b[0m';
               let gray = (x) => C.toANSIFG('gray') + x + '\x1b[0m';
               let output = '';
               
               output += `\n   ${gray('preview:')} ${t?preview:'██'}`;
               output += `\n   ${gray('hex:')} ${hex}`;
               output += `\n   ${gray('rgb:')} ${rgb}`;
               output += `\n   ${gray('hsl:')} ${hsl}`;
               output += `\n   ${gray('ANSI foreground:')} ${t?ANSIFG.replace('\x1b','\\x1b'):undefined}`;
               output += `\n   ${gray('ANSI background:')} ${t?ANSIBG.replace('\x1b','\\x1b'):undefined}`;
               output += `\n`;
               
               console.log(output);
               
            } else {
               console.log('ColorTransform.js')
            }
         }
      }
      module.exports = ColorTransform;
   } else {
      global.ColorTransform = ColorTransform;
   }
